...
parser.add_argument('--exp', type=str,
                    default='SCACDClabel2', help='experiment_name')
parser.add_argument('--model', type=str,
                    default='unet_pro', help='model_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs, selfproout, crossproout, entropy = model(volume_batch)
            ...
            consistency_self_pro = self_proloss(selfproout,outputs,entropy)
            consistency_cross_pro = cross_proloss(selfproout,crossproout,outputs,entropy)
            consistency_loss_aux1 = torch.mean( consistency_self_pro )
            consistency_loss_aux2 = torch.mean(consistency_cross_pro)
            consistency_loss = (consistency_loss_aux1+ consistency_loss_aux2) / 2
            loss = supervised_loss + consistency_weight *consistency_loss #consistency_loss
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
if __name__ == "__main__":
    ...
    snapshot_path = "./model/Prostate_{}_{}_labeled/{}".format(
